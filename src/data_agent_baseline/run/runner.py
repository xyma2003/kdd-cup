...
from data_agent_baseline.solvers import solve_task as xyma_solve_task
...
def _run_single_task_core(
    *,
    ...
    if use_xyma:
        ...
        return xyma_solve_task(task, effective_model)
...
def _run_single_task_in_subprocess(
    task_id: str, config: AppConfig, queue: multiprocessing.Queue[Any], use_xyma: bool = False
) -> None:
    try:
        queue.put(
            {
                "ok": True,
                "run_result": _run_single_task_core(task_id=task_id, config=config, use_xyma=use_xyma),
            }
        )
...
def _run_single_task_with_timeout(
    *, task_id: str, config: AppConfig, use_xyma: bool = False
) -> dict[str, Any]:
    timeout_seconds = config.run.task_timeout_seconds
    if timeout_seconds <= 0:
        return _run_single_task_core(task_id=task_id, config=config, use_xyma=use_xyma)

    # xyma mode: use threading timeout to avoid macOS multiprocessing spawn issues
    if use_xyma:
        import threading
        ...
        def _run():
            try:
                result_container["result"] = _run_single_task_core(
                    task_id=task_id, config=config, use_xyma=True
                )
            except BaseException as exc:  # noqa: BLE001
        ...
        if "error" in result_container:
            return _failure_run_result_payload(task_id, result_container["error"])
        return result_container.get("result", _failure_run_result_payload(task_id, "No result"))

    queue: multiprocessing.Queue[Any] = multiprocessing.Queue()
    process = multiprocessing.Process(
        target=_run_single_task_in_subprocess,
        args=(task_id, config, queue, use_xyma),
    )
    process.start()
...
def run_single_task(
    *,
    ...
    if model is None and tools is None:
        run_result = _run_single_task_with_timeout(task_id=task_id, config=config, use_xyma=use_xyma)
    else:
        run_result = _run_single_task_core(task_id=task_id, config=config, model=model, tools=tools, use_xyma=use_xyma)
...
def run_benchmark(
    *,
    ...
    _resolved_run_id = run_id or config.run.run_id
    effective_run_id = resolve_run_id(_resolved_run_id)
